...
target_user="${OPENVPN_TEST_LOGIN_USER:-trap}"
...
wait_for_exec_success "$service_name" "ss -lun | grep -q ':1194'"
...
compose exec -T -e TARGET_USER="$target_user" attacker sh -lc '
  for i in $(seq 1 6); do
    printf "AUTH user=%s pass=wrong\n" "$TARGET_USER" | nc -u -w1 openvpn 1194 >/dev/null 2>&1 || true
    sleep 1
  done
'

for _ in $(seq 1 30); do
  if compose exec -T fail2ban fail2ban-client status openvpn | grep -F "$attacker_ip" >/dev/null; then
    break
  fi
  sleep 2
done
