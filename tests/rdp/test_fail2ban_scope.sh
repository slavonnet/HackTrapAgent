...
target_user="${RDP_TEST_LOGIN_USER:-trap}"
...
wait_for_exec_success "$service_name" "pgrep -x xrdp"
...
compose exec -T -e TARGET_USER="$target_user" attacker sh -lc '
  for i in $(seq 1 6); do
    hydra -I -t 1 -f -l "$TARGET_USER" -p wrongpass rdp://rdp:3389 >/dev/null 2>&1 || true
    sleep 1
  done
'

for _ in $(seq 1 30); do
  if compose exec -T fail2ban fail2ban-client status xrdp-sesman | grep -F "$attacker_ip" >/dev/null; then
    break
  fi
  sleep 2
done
