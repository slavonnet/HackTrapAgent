...
target_user="${RABBITMQ_TEST_LOGIN_USER:-trap}"
...
compose exec -T -e TARGET_USER="$target_user" attacker sh -lc '
  for i in $(seq 1 6); do
    amqp-publish \
      -u "amqp://${TARGET_USER}:wrong@rabbitmq:5672/%2f" \
      -r "hacktrap.auth.probe" \
      -b "test" >/dev/null 2>&1 || true
    sleep 1
  done
'

for _ in $(seq 1 30); do
  if compose exec -T fail2ban fail2ban-client status rabbitmq | grep -F "$attacker_ip" >/dev/null; then
    break
  fi
  sleep 2
done
