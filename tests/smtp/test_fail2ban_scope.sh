...
wait_for_exec_success "$service_name" "pgrep -f smtp_server.py"
...
for _ in $(seq 1 30); do
  if compose exec -T fail2ban fail2ban-client status hacktrap-smtp | grep -F "$attacker_ip" >/dev/null; then
    break
  fi
  sleep 2
done
