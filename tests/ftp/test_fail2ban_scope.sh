...
target_user="${FTP_TEST_LOGIN_USER:-trap}"
...
wait_for_exec_success "$service_name" "pgrep -x vsftpd"
...
compose exec -T -e TARGET_USER="$target_user" attacker sh -lc '
  for i in $(seq 1 6); do
    lftp -u "${TARGET_USER},wrong" "ftp://ftp" -e "set net:max-retries 1; set net:timeout 3; ls; bye" >/dev/null 2>&1 || true
    sleep 1
  done
'

for _ in $(seq 1 30); do
  if compose exec -T fail2ban fail2ban-client status vsftpd | grep -F "$attacker_ip" >/dev/null; then
    break
  fi
  sleep 2
done
