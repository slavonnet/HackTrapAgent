...
target_user="${TELNETD_TEST_LOGIN_USER:-root}"
...
for _ in $(seq 1 30); do
  if compose exec -T fail2ban fail2ban-client status telnetd | grep -F "$attacker_ip" >/dev/null; then
    break
  fi
  sleep 2
done
...
if ! compose exec -T fail2ban fail2ban-client status telnetd | grep -F "$attacker_ip" >/dev/null; then
  ...
  compose exec -T "$service_name" sh -lc "tail -n 50 /var/log/telnet/auth.log || true"
  exit 1
fi
