...
target_user="${POP3_TEST_LOGIN_USER:-trap}"
  ...
  for i in $(seq 1 6); do
    {
      ...
      printf "PASS wrong\r\n"
      printf "QUIT\r\n"
    } | nc -w 3 pop3 110 >/dev/null 2>&1 || true
    sleep 1
  done
'

for _ in $(seq 1 30); do
  if compose exec -T fail2ban fail2ban-client status dovecot-pop3 | grep -F "$attacker_ip" >/dev/null; then
    break
  fi
  sleep 2
done
