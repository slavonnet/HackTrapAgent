...
target_user="${MONGODB_TEST_LOGIN_USER:-trap}"
...
wait_for_exec_success "$service_name" "pgrep -x mongod"
...
compose exec -T -e TARGET_USER="$target_user" attacker sh -lc '
  for i in $(seq 1 6); do
    TARGET_USER="$TARGET_USER" python3 - <<'"'"'PY'"'"'
import os
from pymongo import MongoClient

target_user = os.environ["TARGET_USER"]
uri = f"mongodb://{target_user}:wrong@mongodb:27017/admin?authSource=admin&serverSelectionTimeoutMS=2000"
try:
...
for _ in $(seq 1 30); do
  if compose exec -T fail2ban fail2ban-client status mongodb-auth | grep -F "$attacker_ip" >/dev/null; then
    break
  fi
  sleep 2
done
